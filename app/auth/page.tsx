      ...
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <span className="text-zinc-800 font-bold text-xl">KadeAI Studio</span>
          </div>
        ...
        <div className="rounded-2xl border border-orange-200 bg-white shadow-lg shadow-orange-100/50 p-6 space-y-5">
            ...
            {(['login', 'signup'] as Mode[]).map((m) => (
              <button key={m} onClick={() => { setMode(m); setError(''); setSuccess('') }}
                className={cn('flex-1 py-1.5 rounded-lg text-sm font-semibold transition-colors',
                  mode === m ? 'bg-orange-500 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700')}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode === 'signup' && (
              <div>
                <label className="block text-zinc-600 text-xs font-medium mb-1.5">Görünen Ad</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                  <input value={nickname} onChange={(e) => setNickname(e.target.value)}
                    placeholder="Kade, Studio Kade..." autoComplete="name"
                    className="w-full bg-orange-50 border border-orange-200 rounded-xl pl-9 pr-3 py-2.5 text-sm text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:border-orange-400 focus:bg-white transition-colors" />
                </div>
              </div>
            )}
            ...
            {/* Şifre */}
            <div>
              <label className="block text-zinc-600 text-xs font-semibold mb-1.5">Şifre</label>
              <div className="relative">
          ...
          <button onClick={handleGoogleLogin}
            className="w-full py-2.5 rounded-xl border border-orange-200 bg-orange-50 text-zinc-700 text-sm font-semibold hover:bg-orange-100 transition-colors flex items-center justify-center gap-2">
